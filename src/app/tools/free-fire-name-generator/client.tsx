...
const FF_BORDERS = [
  ...GAMING_BORDERS,
  { label: "꧁ Name ꧂", wrap: (n: string) => `꧁${n}꧂` },
  { label: "彡 Name 彡", wrap: (n: string) => `彡${n}彡` },
  { label: "ᴳᵒᵈ + Name", wrap: (n: string) => `ᴳᵒᵈ${n}` },
  { label: "Name + FF", wrap: (n: string) => `${n}ッFF` },
];
    ...
    <div className="space-y-6">
      <div>
        ...
        <input
          ...
          placeholder="Enter your FF name…"
          maxLength={12}
        ...
        />
        <p className="mt-1 text-[11px] text-neutral-400">Free Fire allows up to 12 characters in-game (special symbols may count as 1).</p>
      </div>

      <div>
        <h2 className="text-[13px] font-semibold text-foreground mb-3">FF Styled Names</h2>
      ...
      <div>
        <h2 className="text-[13px] font-semibold text-foreground mb-3">FF Border Styles</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          {FF_BORDERS.map((border) => {
            const decorated = border.wrap(input || "YourName");
