...
import { FilePdf, DownloadSimple, CircleNotch, CaretLeft, X, ArrowCounterClockwise } from "@phosphor-icons/react";
...
function hx(id: string, x1: number, x2: number) {
  if (id === "x1") return x1; if (id === "x2") return x2; return (x1 + x2) / 2;
}
function hy(id: string, y1: number, y2: number) {
  if (id === "y1") return y1; if (id === "y2") return y2; return (y1 + y2) / 2;
}

export default function CropPdfClient() {
  const inputRef = useRef<HTMLInputElement>(null);
  const cropBoxesRef = useRef<CropBox[]>([]);
  const [file, setFile] = useState<File | null>(null);
  const [thumbnails, setThumbnails] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  const [progress, setProgress] = useState(0);
  const [cropBoxes, _setCropBoxes] = useState<CropBox[]>([]);
  const [processing, setProcessing] = useState(false);
  const [result, setResult] = useState<{ blob: Blob; url: string } | null>(null);
  const [error, setError] = useState<string | null>(null);

  const setCropBoxes = (b: CropBox[]) => { cropBoxesRef.current = b; _setCropBoxes(b); };
  const updateCropBox = (idx: number, box: CropBox) => {
    const next = [...cropBoxesRef.current];
    next[idx] = box;
    setCropBoxes(next);
  };

  const loadFile = useCallback(async (f: File) => {
    setFile(f); setResult(null); setError(null); setThumbnails([]); setCropBoxes([]);
    ...
    try {
      ...
      for (let i = 1; i <= total; i++) {
        ...
        items.push(canvas.toDataURL("image/jpeg", 0.88));
        page.cleanup();
        setProgress(Math.round((i / total) * 100));
      }
      setThumbnails(items);
      setCropBoxes(items.map(() => ({ ...DEFAULT_BOX })));
    } catch (e) {
  ...
  const startDrag = (e: React.PointerEvent<HTMLElement>, handle: string, pageIdx: number) => {
    e.preventDefault();
    e.stopPropagation();
    const card = (e.currentTarget as HTMLElement).closest("[data-crop-card]") as HTMLElement;
    const rect = card.getBoundingClientRect();
    const startX = e.clientX, startY = e.clientY;
    const startBox = { ...cropBoxesRef.current[pageIdx] };
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      updateCropBox(pageIdx, { x1, y1, x2, y2 });
    };
  ...
  const process = async () => {
    ...
    try {
      ...
      doc.getPages().forEach((page, i) => {
        const { x1, y1, x2, y2 } = cropBoxes[i] ?? DEFAULT_BOX;
        ...
        <div className="flex flex-col sm:flex-row min-h-[calc(100vh-4rem)]">
            ...
            {!loading && thumbnails.length > 0 && (
              <div className="space-y-8 max-w-xl mx-auto">
                {thumbnails.map((thumb, idx) => {
                  ...
                  const isCropped = x1 > 0.001 || y1 > 0.001 || x2 < 0.999 || y2 < 0.999;
                  return (
                    <div key={idx} className="space-y-2">
                      {/* Page label + reset */}
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-medium text-muted-foreground">Page {idx + 1}</span>
                        {isCropped && (
                          <button
                            onClick={() => updateCropBox(idx, { ...DEFAULT_BOX })}
                      ...
                      <div
                        data-crop-card
                        className="relative overflow-hidden ring-1 ring-black/10 select-none cursor-crosshair"
                      >
                        ...
                        {HANDLES.map(h => (
                          <div
                            ...
                            style={{
                              left: `${hx(h.cx, x1, x2) * 100}%`,
                              top: `${hy(h.cy, y1, y2) * 100}%`,
                              cursor: h.cursor,
                            }}
                            onPointerDown={e => startDrag(e, h.id, idx)}
                          />
                        ))}
                      </div>
                    </div>
          ...
          <div className="w-full sm:w-80 shrink-0 border-t sm:border-t-0 sm:border-l border-border bg-white sm:sticky sm:top-16 sm:h-[calc(100vh-4rem)] flex flex-col p-6 gap-4 overflow-y-auto">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setFile(null); setThumbnails([]); setResult(null); setCropBoxes([]); }}
            ...
            <div className="space-y-3">
              ...
              {[
                { n: "1", text: "Drag the white handles on each page to resize its crop area." },
                { n: "2", text: "Drag inside the bright region to reposition the crop box." },
                { n: "3", text: "Each page can have a different crop." },
              ].map(({ n, text }) => (
            ...
            <button
              onClick={() => setCropBoxes(thumbnails.map(() => ({ ...DEFAULT_BOX })))}
              className="flex items-center gap-1.5 text-left text-[12px] text-muted-foreground hover:text-foreground transition-colors"
            >
              <ArrowCounterClockwise size={12} />Reset all crops
            </button>
