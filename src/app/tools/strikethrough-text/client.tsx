...
import { withCombining } from "@/lib/unicode-text";

const VARIANTS: { id: string; name: string; convert: (t: string) => string }[] = [
  { id: "strike", name: "Strikethrough", convert: (t) => withCombining(t, "̶") },
  { id: "underline", name: "Underline", convert: (t) => withCombining(t, "̲") },
  { id: "double-strike", name: "Double Strikethrough", convert: (t) => withCombining(t, "̸") },
  { id: "overline", name: "Overline", convert: (t) => withCombining(t, "̄") },
];

export function StrikethroughTextClient() {
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {VARIANTS.map((variant) => {
          const preview = input ? variant.convert(input) : variant.convert(variant.name);
          ...
          return (
            ...
            >
              <p className="text-[11px] font-medium text-neutral-400 mb-1.5">{variant.name}</p>
              <p className="text-[14px] leading-relaxed break-words pr-20 text-foreground">
                {preview}
              </p>
              <button
                onClick={() => copy(variant.id, preview)}
