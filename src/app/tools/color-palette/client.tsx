...
interface PickDot { relX: number; relY: number; hex: string; id: string; }
...
function exportPalettePng(palette: ColorEntry[]): void {
  ...
  palette.forEach((c, i) => {
    ctx.fillStyle = c.hex; ctx.fillRect(i * 80, 0, 80, 70);
    ctx.fillStyle = "#ffffff"; ctx.fillRect(i * 80, 70, 80, 30);
    ctx.fillStyle = "#333333"; ctx.font = "bold 11px system-ui, sans-serif";
    ctx.textAlign = "center"; ctx.fillText(c.hex.toUpperCase(), i * 80 + 40, 89);
  });
  canvas.toBlob(blob => {
    if (!blob) return;
...
function buildCssVars(palette: ColorEntry[]): string {
  return `:root {\n${palette.map((c, i) => `  --color-${i + 1}: ${c.hex};`).join("\n")}\n}`;
}

function buildTailwindConfig(palette: ColorEntry[]): string {
  const entries = palette.map((c, i) => `        "${i + 1}": "${c.hex}",`).join("\n");
  return `// tailwind.config.js\nmodule.exports = {\n  theme: {\n    extend: {\n      colors: {\n        palette: {\n${entries}\n        },\n      },\n    },\n  },\n};`;
}

function buildJson(palette: ColorEntry[]): string {
  return JSON.stringify({ colors: palette.map(c => c.hex) }, null, 2);
}

// ─── Harmony ─────────────────────────────────────────────────────────────────

function getHarmony(hex: string) {
...
function PalettePanel({
  ...
  const copyColor = async (c: ColorEntry) => {
    ...
    setCopiedIdx(c.id); setTimeout(() => setCopiedIdx(null), 1500);
  };
  const copyAll = async () => {
  ...
  const copyExport = async (type: "css" | "tailwind" | "json") => {
    const text = type === "css" ? buildCssVars(palette) : type === "tailwind" ? buildTailwindConfig(palette) : buildJson(palette);
    await navigator.clipboard.writeText(text);
    setCopiedExport(type); setTimeout(() => setCopiedExport(null), 1800);
  };
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-black/6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
      ...
      <div className="flex items-center gap-1.5 border-b border-border px-4 py-2">
        <span className="shrink-0 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">Export:</span>
        {(["css", "tailwind", "json"] as const).map(type => (
          <button key={type} onClick={() => copyExport(type)}
            className={cn(
            ...
            {copiedExport === type
              ? <><Check size={10} weight="bold" />Copied</>
              : type === "css" ? "CSS vars" : type === "tailwind" ? "Tailwind" : "JSON"}
          </button>
        ))}
      </div>
            ...
            <div key={c.id} className="flex items-center gap-3 px-4 py-3 group">
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button onClick={() => copyColor(c)} className="text-neutral-300 transition-colors hover:text-neutral-500">
                  {copiedIdx === c.id ? <Check size={14} weight="bold" className="text-emerald-500" /> : <Copy size={14} />}
                </button>
                {onRemove && (
                  <button onClick={() => onRemove(c.id)} className="ml-0.5 text-neutral-200 transition-colors hover:text-red-400">
                    <X size={13} />
                  </button>
                )}
              </div>
...
function HarmonyPanel({ baseColor }: { baseColor: ColorEntry }) {
  const [copied, setCopied] = useState<string | null>(null);
  const harmony = getHarmony(baseColor.hex);
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-black/6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
      ...
      <div className="divide-y divide-border">
        {groups.map(group => (
          <div key={group.label} className="flex items-center gap-3 px-4 py-2.5">
            <span className="w-28 shrink-0 text-[12px] text-muted-foreground">{group.label}</span>
            <div className="flex flex-1 gap-2">
              {group.colors.map(hex => {
                const [r, g, b] = hexToRgbTriple(hex);
                return (
                  <button key={hex} onClick={() => copyHex(hex)}
                    title={hex.toUpperCase()}
                    className="relative flex h-8 flex-1 items-center justify-center rounded-lg ring-1 ring-black/10 transition-opacity hover:opacity-80"
                    style={{ backgroundColor: hex }}>
                    {copied === hex && <Check size={12} weight="bold" style={{ color: getTextColor(r, g, b) }} />}
                  </button>
                );
              })}
            </div>
            <div className="flex flex-col gap-0.5">
              {group.colors.map(hex => (
                <span key={hex} className="font-mono text-[10px] text-muted-foreground/60">{hex.toUpperCase()}</span>
              ))}
            </div>
          </div>
        ))}
...
export function ColorPaletteClient() {
  ...
  const [picked, setPicked]         = useState<ColorEntry[]>([]);
  ...
  const clearPicked = useCallback(() => { setPicked([]); setPickDots([]); }, []);
      ...
      {!file && (
        ...
        >
          ...
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-muted-foreground">auto-extract · click to pick colours · export CSS / JSON</p>
          </div>
      ...
      {file && imgUrl && (
        <>
          <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-black/6 shadow-[0_4px_24px_-6px_rgba(0,0,0,0.10),0_1px_3px_rgba(0,0,0,0.06)]">
            {activePalette.length > 0 && (
              <div className="flex h-8 w-full overflow-hidden">
                {activePalette.map(c => (
                  <div key={c.id} className="transition-all duration-300" style={{ backgroundColor: c.hex, flexGrow: c.count }} />
                ))}
              </div>
            )}
            ...
            <div className="flex border-b border-border">
              {(["extract", "pick"] as Mode[]).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  className={cn(
                    "flex-1 py-2 text-[12px] font-medium transition-colors",
                    mode === m ? "border-b-2 border-foreground text-foreground" : "text-muted-foreground hover:text-foreground",
                  )}>
                  {m === "extract" ? "Auto extract" : "Click to pick"}
                </button>
              ))}
            </div>
            ...
            >
              ...
              <img src={imgUrl} alt="" className="block max-w-full object-contain" draggable={false} style={{ maxHeight: 340 }} />
              {mode === "pick" && pickDots.map(dot => {
                const [r, g, b] = hexToRgbTriple(dot.hex);
                return (
                  <div key={dot.id}
                    style={{
                      position: "absolute",
                      left: `${dot.relX * 100}%`, top: `${dot.relY * 100}%`,
                      transform: "translate(-50%, -50%)",
                      width: 18, height: 18, borderRadius: "50%",
                      backgroundColor: dot.hex,
                      border: `2px solid ${getTextColor(r, g, b)}`,
          ...
          {mode === "extract" && !extracting && palette.length > 0 && (
            <button onClick={() => imgUrl && runExtraction(imgUrl)}
          ...
          {!extracting && activePalette.length > 0 && (
            <>
              <PalettePanel palette={activePalette} format={format} setFormat={setFormat} onRemove={mode === "pick" ? removePicked : undefined} />
              <HarmonyPanel baseColor={activePalette[0]} />
            </>
          )}
          ...
          {mode === "pick" && picked.length === 0 && (
            <div className="rounded-xl bg-neutral-50 px-4 py-4 text-center ring-1 ring-black/5">
              ...
              <p className="mt-1 text-[12px] text-muted-foreground">Click directly on the image above to sample a colour.</p>
            </div>
          )}
        </>
      )}
